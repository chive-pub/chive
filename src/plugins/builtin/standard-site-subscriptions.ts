...
export class StandardSiteSubscriptionsPlugin extends BasePlugin {
  ...
  readonly manifest: IPluginManifest = {
    ...
    permissions: {
      hooks: [
        ...
        `firehose.${PUBLICATION_COLLECTION}`,
  ...
  protected override async onInitialize(): Promise<void> {
    ...
    this.context.eventBus.on(
      `firehose.${PUBLICATION_COLLECTION}`,
      (...args: readonly unknown[]) => {
        void this.handlePublication(args[0] as FirehoseRecord);
      }
    );

    this.logger.info('Indexing the standard.site social graph', {
      collections: [SUBSCRIPTION_COLLECTION, RECOMMEND_COLLECTION, PUBLICATION_COLLECTION],
    });
  ...
  /**
   * Handles one `site.standard.publication`.
   *
   * @remarks
   * A publication belongs to the repository holding it, so the record's own DID
   * is its author. Indexed for every publication seen, not only Chive's own: a
   * reader following an author's Leaflet blog and their papers is following two
   * publications, and Chive has no business deciding which of them counts.
   */
  async handlePublication(event: FirehoseRecord): Promise<void> {
    if (!this.subscriptions) return;

    try {
      if (event.deleted) {
        await this.subscriptions.deletePublication(event.uri);
        return;
      }

      const name = event.record?.name;
      // `name` is required by the schema; a record without one names nothing a
      // reader could be offered.
      if (typeof name !== 'string' || name.length === 0) return;

      const url = event.record?.url;
      const description = event.record?.description;

      await this.subscriptions.recordPublication({
        uri: event.uri,
        authorDid: event.did,
        name,
      ...
      this.logger.warn('Failed to index publication', {
        uri: event.uri,
        error: err instanceof Error ? err.message : String(err),
      });
    }
