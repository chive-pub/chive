...
export class SubscriptionService {
  ...
  /**
   * The eprints published by the authors a reader subscribes to.
   *
   * @param subscriberDid - The reader
   * @param options - `limit` caps the page; `cursor` continues one
   * @returns Eprints newest first, with a cursor when more remain
   *
   * @remarks
   * Matched on `paper_did` and on the `authors` array, not on `submitted_by`:
   * a paper submitted by a co-author is still that author's paper, and a reader
   * subscribing to someone expects their work regardless of who filed it.
   *
   * The cursor is the `created_at` of the last row, paired with its uri so that
   * eprints sharing a timestamp -- a bulk import, most obviously -- cannot
   * straddle a page boundary and be skipped or repeated.
   */
  async getSubscribedFeed(
    subscriberDid: string,
    options: { readonly limit?: number; readonly cursor?: string } = {}
  ): Promise<{ eprints: { uri: string; title: string; createdAt: Date }[]; cursor?: string }> {
    const limit = Math.min(options.limit ?? 25, 100);

    const dids = await this.getSubscribedDids(subscriberDid);
    if (dids.length === 0) {
      return { eprints: [] };
    }

    const params: unknown[] = [dids, limit + 1];
    let keyset = '';

    if (options.cursor) {
      const [at, uri] = options.cursor.split('|');
      if (at && uri) {
        params.push(new Date(at), uri);
        keyset = `AND (e.created_at, e.uri) < ($3, $4)`;
      }
    }

    const result = await this.db.query<{ uri: string; title: string; created_at: Date }>(
      `SELECT e.uri, e.title, e.created_at
       ...
       WHERE e.deleted_at IS NULL
         ...
         ${keyset}
       ORDER BY e.created_at DESC, e.uri DESC
       LIMIT $2`,
      params
    );
    ...
    return {
      eprints: rows.map((row) => ({
        uri: row.uri,
        title: row.title,
        createdAt: row.created_at,
      })),
      ...(hasMore && last ? { cursor: `${last.created_at.toISOString()}|${last.uri}` } : {}),
    };
  }
