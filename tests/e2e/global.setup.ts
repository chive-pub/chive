...
const TEST_ENRICHMENTS = [
  {
    ...
    topics: [
      ...
      {
        id: 'T101',
        displayName: 'Verb Selectional Restrictions',
...
const TEST_PROPOSALS = [
  // Field proposals
  {
    ...
    rationale:
      'Emerging interdisciplinary field combining quantum computing principles with linguistic theory. Multiple papers published in past 3 years.',
...
async function seedPostgres(): Promise<void> {
  ...
  try {
    ...
    if (backlinksTableCheck.rows[0].exists) {
      ...
      // Update backlink counts using the stored function
 ...
 *
 * @remarks
 * Uses Elasticsearch data streams (industry standard since ES 7.9+).
 * Data streams require:
 * - `@timestamp` field on all documents
 * - `create` action in bulk operations (not `index`)
 * - Data stream template must be set up first (via bootstrapDataStream)
 */
async function seedElasticsearch(): Promise<void> {
  ...
  try {
    ...
    // Ensure data stream exists (created by API startup or test setup)
    ...
    if (!dataStreamExists) {
      // Import and run Elasticsearch setup to create template and data stream
    ...
    const operations = TEST_PREPRINTS.flatMap((p) => {
      ...
      return [
        ...
        { create: { _index: 'preprints' } },
        {
          ...
          // Core fields matching template schema
          uri: p.uri,
          cid: p.cid,
          ...
          abstract: p.abstract,
          keywords: p.keywords,
          // Nested author object matching template
          authors: [
            {
...
async function seedNeo4j(): Promise<void> {
  ...
  try {
    ...
    // Create fields with approved status (required for API filtering)
    // Note: Must set all required properties: id, label, status, createdAt, updatedAt
