...
export function createMockReviewService(): ServerConfig['reviewService'] {
  return {
    ...
    listReviewsOnAuthorPapers: vi.fn().mockResolvedValue({ items: [], hasMore: false, total: 0 }),
    listEndorsementsOnAuthorPapers: vi
      .fn()
...
export function createMockClaimingService(): ServerConfig['claimingService'] {
  return {
    ...
    requestCoauthorship: vi.fn().mockResolvedValue({
      id: 1,
      eprintUri: 'at://did:plc:test/pub.chive.eprint.submission/123',
      eprintOwnerDid: 'did:plc:owner',
      claimantDid: 'did:plc:test',
