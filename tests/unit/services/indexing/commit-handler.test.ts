...
describe('CommitHandler', () => {
  ...
  describe('parsePath', () => {
    it('extracts collection and rkey from valid path', () => {
      const result = handler.parsePath('pub.chive.preprint.submission/abc123');

      expect(result.collection).toBe('pub.chive.preprint.submission');
      expect(result.rkey).toBe('abc123');
    });

    it('handles various rkey formats', () => {
      const result1 = handler.parsePath('pub.chive.preprint.submission/3kj5h2k3j5h');
    ...
    it('throws on path without slash', () => {
      expect(() => handler.parsePath('pub.chive.preprint.submission')).toThrow(
    ...
    it('rejects paths with multiple slashes', () => {
      ...
      expect(() => handler.parsePath('pub.chive.preprint.submission/abc/def')).toThrow(
        'Invalid operation path format'
      );
    });
  });
    ...
    it('handles events with no blocks', async () => {
      const event = {
        ...
        ops: [
          {
            action: 'create' as const,
            path: 'pub.chive.preprint.submission/xyz',
