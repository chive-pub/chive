...
Chive uses the `pub.chive.*` namespace for all AT Protocol lexicons. These lexicons define 23 record types, 130 queries, and 49 procedures across 26 namespaces. All records are stored in user-controlled PDSes and indexed by the Chive AppView via the ATProto firehose.
