...
pnpm exec lex gen-server --yes "$SERVER_OUTPUT_DIR" $(find "$LEXICONS_DIR" -name "*.json" -not -path "*/auth/*" | xargs)
...
pnpm exec lex gen-api --yes "$WEB_OUTPUT_DIR" $(find "$LEXICONS_DIR" -name "*.json" -not -path "*/auth/*" | xargs)
