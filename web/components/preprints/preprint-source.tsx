...
import type { PreprintSource as PreprintSourceType } from '@/lib/api/schema';

/**
 * Props for the PreprintSource component.
 */
export interface PreprintSourceProps {
  /** Source data including PDS endpoint and verification status */
  source: PreprintSourceType;
  /** Display variant */
  variant?: 'inline' | 'card';
 ...
 *
 * @remarks
 * Server component that shows where the preprint data originates from.
 ...
 *     recordUrl: 'at://did:plc:abc/pub.chive.preprint.submission/123',
 *     stale: false,
 *   }}
 * />
 * ```
 ...
 */
export function PreprintSource({ source, variant = 'inline', className }: PreprintSourceProps) {
  if (variant === 'card') {
    return <PreprintSourceCard source={source} className={className} />;
  }
...
/**
 * Card variant of the PreprintSource component.
 */
function PreprintSourceCard({ source, className }: PreprintSourceProps) {
  return (
    <div
      className={cn(
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            This preprint is stored in the author&apos;s Personal Data Server (PDS).
          </p>
        </div>
      </div>
