...
export interface AnnotatedPDFViewerProps {
  ...
  did: string;
  /** AT-URI of the preprint (for annotations) */
  preprintUri: string;
...
export function AnnotatedPDFViewer({
  ...
  const { data: inlineReviewsData } = useInlineReviews(preprintUri, {
  ...
  }, [currentSelection, onAddReview, preprintUri]);
  ...
  const handleLinkEntity = useCallback(() => {
    ...
    setCurrentSelection(null);
    highlighterUtilsRef.current?.setTip(null);
  }, [currentSelection, onLinkEntity, preprintUri]);
