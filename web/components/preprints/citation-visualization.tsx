 ...
 *
 * @remarks
 * Renders citation relationships for a preprint as an interactive node graph.
 * Shows papers that cite this preprint and papers this preprint references.
 *
 * @example
 * ```tsx
 * <CitationVisualization preprintUri="at://did:plc:.../pub.chive.preprint.submission/abc" />
 * ```
 *
...
export interface CitationVisualizationProps {
  /** Preprint URI to visualize citations for */
  preprintUri: string;
...
function layoutCitationNodes(
  centerPreprint: { uri: string; title: string },
  ...
  // Center node
  nodes.push({
    id: centerPreprint.uri,
    type: 'citation',
    position: { x: 0, y: 0 },
    data: {
      label:
        centerPreprint.title.length > 50
          ? centerPreprint.title.substring(0, 50) + '...'
          : centerPreprint.title,
      type: 'center',
      uri: centerPreprint.uri,
    },
  ...
  const citingPapers = citations.filter((c) => c.citedUri === centerPreprint.uri);
  const referencedPapers = citations.filter((c) => c.citingUri === centerPreprint.uri);
  ...
  citingPapers.slice(0, 10).forEach((citation, index) => {
    ...
    edges.push({
      id: `${nodeId}-${centerPreprint.uri}`,
      source: nodeId,
      target: centerPreprint.uri,
      type: 'smoothstep',
  ...
  referencedPapers.slice(0, 10).forEach((citation, index) => {
    ...
    edges.push({
      id: `${centerPreprint.uri}-${nodeId}`,
      source: centerPreprint.uri,
      target: nodeId,
      type: 'smoothstep',
      animated: false,
 ...
 */
function useCitations(preprintUri: string) {
    ...
    async function fetchCitations() {
      try {
        ...
        const response = await fetch(
          `/xrpc/pub.chive.discovery.getCitations?uri=${encodeURIComponent(preprintUri)}&direction=both&limit=20`
        );
...
export function CitationVisualization({
  ...
  const { data, isLoading, error } = useCitations(preprintUri);

  const { initialNodes, initialEdges } = useMemo(() => {
    if (!data) return { initialNodes: [], initialEdges: [] };

    const { nodes, edges } = layoutCitationNodes(data.preprint, data.citations, data.counts);
  ...
  const onNodeClick = useCallback(
    (_event: React.MouseEvent, node: Node<CitationNodeData>) => {
      if (node.id !== preprintUri && node.data.uri) {
        // Navigate to the clicked preprint
        const rkey = node.data.uri.split('/').pop();
        if (rkey) {
          router.push(`/preprints/${encodeURIComponent(node.data.uri)}`);
        }
      }
    },
    [preprintUri, router]
